...
const RewardCard = ({ title, pledge, description, left }: RewardCardProps) => {
  ...
  return (
    <section
      className={`mt-6 rounded-xl border px-8 py-10
        ${
          isOutOfStock
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-xl font-bold">{title}</h3>
        {/* <span className="text-sm font-medium text-teal-500">{pledge}</span> */}
        <span
          className={`text-sm font-medium
    ${isOutOfStock ? "text-teal-300" : "text-sm font-medium text-teal-500"}`}
        >
          {pledge}
        </span>
      </div>
      ...
      <div className="mt-6 flex items-center justify-between">
        ...
        <button
          ...
          className={`rounded-full px-8 py-3 text-sm text-white font-bold
            ${
              isOutOfStock
